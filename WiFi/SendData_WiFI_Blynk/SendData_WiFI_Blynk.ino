/*This script will send data to ThingsBoard using the Panther Buoy's WiFi modem
...
char pass[] = "xxxxlaxxx";                                                       // network password 
...
void ThingBoard() {                                                                   //Make a function to send data to the ThingsBoard
  ...
  if (client.connect(server, 80)) {
    ...
    client.println("POST /api/v1/7grkwum76g2m39uu8dru/telemetry HTTP/1.1");            //This is the header information
    client.println("Host: thingsboard.cloud");                                         //Note that each header component is on a new line with println, not print
    client.println("User-Agent: Panther Buoy/1.0");
...
void setup() {
  ...
  WiFi.setPins(9,7,3,-1);                                                                //Tell WiFi101 library the WiFi pins needed for communications on the Panther Buoy
