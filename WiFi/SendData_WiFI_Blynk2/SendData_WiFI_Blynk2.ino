...
char pass[] = "xxxxhlxxx";    // network password 

int status = WL_IDLE_STATUS; 

//Create C-Stings (character arrays) to hold necessary items for communication
char server[] = "ny3.blynk.cloud";             //server address we will connect to at ThingSpeak
char ThingString[500];                            //character array to hold the url and data... this is the GET url
char ThingAPIKey[] = "3hX7F58zFL5S51vaoq-fsMPgMxSlU5dH";          //String to hold the api key we get from ThingSpeak. Change to your key, gotten from the channel settings page
...
void ThingSpeak() {
  ...
  sprintf(ThingString, "GET blynk.cloud/api/update?token=%s&V0=%.2f&V1=%.2f&V2=%.2f&V3=%.2f&V4=%d&V5=%.2f&V6=%.2f&V7=%.2f",
  ...
  //If we are connected then we can send the data
    ...
    delay(5000); //Wait a second before reading server response
